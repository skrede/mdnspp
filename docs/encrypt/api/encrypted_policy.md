...
Composable policy wrapper that substitutes `encrypted_socket` for the inner policy's socket type. Satisfies the `Policy` concept, so any `basic_*` class template that accepts a policy works with `encrypted_policy<Inner>` without modification.
...
|-----------|------------|-------------|
| `Inner` | satisfies `Policy` | The underlying policy providing the executor, timer, and inner socket. Typically `DefaultPolicy`, `AsioPolicy`, or `InProcPolicy`. |
...
`encrypted_policy<Inner>` satisfies the `Policy` concept because:

- `executor_type`, `socket_type`, and `timer_type` are present.
- `socket_type` (`encrypted_socket<Inner::socket_type>`) satisfies `SocketLike`.
- `timer_type` satisfies `TimerLike` (same type as `Inner::timer_type`).
...
// Construct an observer using the encrypted policy over DefaultPolicy
mdnspp::encrypt_socket_options opts;
opts.encrypt.psk       = mdnspp::secure_key{my_key};
opts.encrypt.sender_id = 1;

mdnspp::context ctx;
mdnspp::basic_observer<mdnspp::encrypted_policy<mdnspp::DefaultPolicy>> obs{
    ctx,
...
- [defaults](defaults.md) -- convenience aliases (`encrypted_observer`, etc.) using `encrypted_policy<DefaultPolicy>`
- [Custom Policies](../../custom-policies.md) -- Policy concept requirements
