...
mdnspp::encrypted_packet_header make_header(uint32_t sender = 1,
                                             uint64_t seq    = 1,
                                             uint32_t epoch  = 0,
                                             uint8_t  flags  = mdnspp::flag_encrypted)
{
    mdnspp::encrypted_packet_header h;
    h.sender_id = sender;
    h.sequence  = seq;
    h.epoch     = epoch;
    h.flags     = flags;
...
TEST_CASE("aead roundtrip returns original plaintext", "[aead][roundtrip]")
{
    ...
    auto packet = mdnspp::aead_encrypt(key, hdr, payload);
    auto result = mdnspp::aead_decrypt(key, packet);
...
TEST_CASE("aead roundtrip with empty plaintext", "[aead][roundtrip]")
{
    ...
    auto packet = mdnspp::aead_encrypt(key, hdr, empty);
    auto result = mdnspp::aead_decrypt(key, packet);
...
TEST_CASE("aead output size is header + plaintext + tag", "[aead][size]")
{
    ...
    const std::size_t expected_size =
        mdnspp::encrypted_header_size + payload.size() + mdnspp::encrypted_tag_size;
...
TEST_CASE("aead output size for empty plaintext is exactly encrypted_overhead", "[aead][size]")
{
    ...
    auto packet = mdnspp::aead_encrypt(key, hdr, empty);
    REQUIRE(packet.size() == mdnspp::encrypted_overhead);
}
...
TEST_CASE("aead two encrypts with same key+plaintext produce different ciphertexts", "[aead][nonce]")
{
    ...
    auto packet1 = mdnspp::aead_encrypt(key, hdr, payload);
    auto packet2 = mdnspp::aead_encrypt(key, hdr, payload);
...
TEST_CASE("flipping a byte in the auth tag causes decrypt_failed", "[aead][tamper]")
{
    ...
    REQUIRE(result.error() == mdnspp::encrypt_error::decrypt_failed);
}
...
TEST_CASE("flipping a byte in the ciphertext region causes decrypt_failed", "[aead][tamper]")
{
    ...
    packet[mdnspp::encrypted_header_size] ^= std::byte{0xFF};
...
TEST_CASE("wrong magic returns invalid_header", "[aead][header]")
{
    ...
    auto result = mdnspp::aead_decrypt(key, packet);
    REQUIRE_FALSE(result.has_value());
    REQUIRE(result.error() == mdnspp::encrypt_error::invalid_header);
}
...
TEST_CASE("wrong version returns unsupported_version", "[aead][header]")
{
    ...
    auto result = mdnspp::aead_decrypt(key, packet);
    REQUIRE_FALSE(result.has_value());
    REQUIRE(result.error() == mdnspp::encrypt_error::unsupported_version);
}
...
TEST_CASE("too-short packet returns invalid_header", "[aead][header]")
{
    ...
    std::vector<std::byte> short_packet(mdnspp::encrypted_overhead - 1, std::byte{0});

    auto result = mdnspp::aead_decrypt(key, short_packet);
    REQUIRE_FALSE(result.has_value());
    REQUIRE(result.error() == mdnspp::encrypt_error::invalid_header);
}
...
TEST_CASE("secure_key move-assign zeroes source and previous destination bytes", "[secure_key][move]")
{
    ...
    mdnspp::secure_key a{raw_a};
    mdnspp::secure_key b{raw_b};
...
TEST_CASE("encrypt_options validate succeeds with non-zero sender_id", "[encrypt_options][validate]")
{
    ...
    opts.psk       = mdnspp::secure_key{raw};
    opts.sender_id = 1;
...
TEST_CASE("auth-only mode preserves cleartext payload in output", "[aead][auth-only]")
{
    ...
    REQUIRE(packet.size() == mdnspp::encrypted_header_size + payload.size() + mdnspp::encrypted_tag_size);

    // The payload region must match the original plaintext (cleartext, not encrypted)
    std::span<const std::byte> payload_region(
        packet.data() + mdnspp::encrypted_header_size,
        payload.size());
