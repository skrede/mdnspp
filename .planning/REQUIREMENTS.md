...
- [ ] **API-03**: Fallible construction uses `std::expected` factory functions (`static std::expected<T, mdns_error> create(...)` replaces throwing constructors; `[[nodiscard]]` enforced)
...
- [ ] **API-05**: Buffer API uses `std::span<std::byte>` (caller-owned buffers; no raw pointer/size pairs in public API)
...
- [ ] **ARCH-04**: `AsioSocketPolicy` provides production async I/O (standalone ASIO 1.30.x; `asio::ip::udp::socket`; multicast join sequence; `async_receive_from` chain; `ASIO_STANDALONE` defined; no Boost dependency)
...
- [ ] **ARCH-06**: `recv_loop<S>` implemented as a standalone internal component (owns receive buffer as `std::vector<std::byte>`; passes `std::span<std::byte>` to policy; silence detection via `asio::steady_timer`; stop is idempotent)
- [ ] **ARCH-07**: `record_parser` implemented as free functions in `mdnspp::parse` namespace (`std::span<const std::byte>` + endpoint → `mdns_record_variant`; `mdns.h` confined to `src/`; testable without sockets)

### Behavior — Runtime Semantics

- [ ] **BEHAV-01**: `discover()` and `query()` return accumulated results (`std::expected<std::vector<mdns_record_variant>, mdns_error>`; per-record callback removed as primary interface)
- [ ] **BEHAV-02**: Executor injected at construction and hidden from public mDNS methods (user passes `asio::any_io_executor` or `asio::io_context&` once at construction; `announce()`, `discover()`, `query()`, `observe()` have no executor parameters)
...
- [ ] **TEST-01**: Unit tests for all four operation types via `MockSocketPolicy` (inject known DNS wire bytes, verify accumulated results; no real network required)
- [ ] **TEST-02**: `record_parser` unit tests with raw byte spans (PTR, SRV, TXT, A, AAAA records; edge cases: truncated names, unknown record types, TXT key-value parsing)
- [ ] **TEST-03**: `recv_loop` unit tests (async receive chain, stop idempotency, silence detection, mock packet injection and delivery)
...
- [ ] **BUILD-02**: Standalone ASIO added as optional FetchContent dependency (`MDNSPP_ENABLE_ASIO_POLICY=ON` CMake option; `ASIO_STANDALONE` defined on all targets that include ASIO headers; fetched only when enabled)
...
| BEHAV-02 | Phase 2 | Pending |
| BUILD-02 | Phase 2 | Pending |
| TEST-03 | Phase 2 | Pending |
| ARCH-07 | Phase 3 | Pending |
| API-05 | Phase 3 | Pending |
| TEST-02 | Phase 3 | Pending |
| BEHAV-01 | Phase 4 | Pending |
...
*Last updated: 2026-03-03 — TEST-04 split (MockSocketPolicy Phase 1 complete, AsioSocketPolicy Phase 2 pending); API-01 scope annotated; legacy headers removed from PUBLIC_HEADERS after 01-05 gap closure*
