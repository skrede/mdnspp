...
- [x] **AGG-01**: `resolved_service` struct combining service name, hostname, port, TXT records, and IP addresses
- [x] **AGG-02**: Correlation logic aggregates PTR → SRV → TXT → A/AAAA records into `resolved_service`
- [ ] **AGG-03**: Discovery can return `resolved_service` instead of raw `mdns_record_variant`
- [x] **AGG-04**: Aggregation handles partial responses (service discovered without all record types yet)
...
| AGG-01 | Phase 11 | Complete |
| AGG-02 | Phase 11 | Complete |
| AGG-03 | Phase 11 | Pending |
| AGG-04 | Phase 11 | Complete |
