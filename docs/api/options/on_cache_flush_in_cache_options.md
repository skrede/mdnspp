...
| **One-liner** | Fired when the DNS cache-flush bit triggers eviction of records that conflict with a newly received authoritative record. |

## What

`on_cache_flush` is a `move_only_function` callback invoked when the library processes a record with the DNS cache-flush bit set (RFC 6762 §10.2). When a flush-flagged record is received, existing cached records of the same name and type from a different source are scheduled for eviction after the `goodbye_grace` window, provided they were received more than one second ago (the §10.2 young-record exemption protects bursts arriving via multiple interfaces or co-located responders). This callback delivers:

- The newly received authoritative record (first argument, `const cache_entry&`)
- The vector of records scheduled for eviction by it (second argument, `std::vector<cache_entry>`)
...
- The callback does not fire when the incoming flush record was genuinely new and no existing records were displaced.
