...
- [x] **Phase 9: NativePolicy Standalone Networking** - Raw UDP multicast socket policy with `run()`/`poll()` event loop; no ASIO in include chain; cross-platform (completed 2026-03-04)
- [x] **Phase 10: ASIO Completion Tokens** - Wire `async_initiate` across all four public types; support callback, `use_future`, `use_awaitable`, and `deferred` from one call site (completed 2026-03-04)
- [ ] **Phase 11: Service Aggregation** - `resolved_service` struct correlating PTR+SRV+TXT+A/AAAA records; `async_browse` returns aggregated results in one completion
...
**Plans**: 2 plans
  - [ ] 11-01-PLAN.md — Define resolved_service struct and aggregate() free function with TDD
  - [ ] 11-02-PLAN.md — Wire async_browse into service_discovery with MockPolicy integration tests
...
| 9. NativePolicy Standalone Networking | v2.0 | Complete    | 2026-03-04 | - |
| 10. ASIO Completion Tokens | 2/2 | Complete    | 2026-03-04 | - |
| 11. Service Aggregation | 1/2 | In Progress|  | - |
